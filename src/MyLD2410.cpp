...
bool MyLD2410::processData() {
  ...
  if (!LD2410::bufferEndsWith(inBuf, inBufI, LD2410::tailData))
    return false;
  if (((inBuf[0] == 1) || (inBuf[0] == 2)) && (inBuf[1] == 0xAA)) {  // Basic mode and Enhanced
    ...
    if (inBuf[0] == 1) {  // Enhanced mode only
...
bool MyLD2410::stationaryTargetDetected() {
  return isDataValid() && ((sData.status == 2) || (sData.status == 3)) && (sData.sTargetDistance) && (sData.sTargetSignal);
}
...
bool MyLD2410::movingTargetDetected() {
  return isDataValid() && ((sData.status == 1) || (sData.status == 3)) && (sData.mTargetDistance) && (sData.mTargetSignal);
}
...
bool MyLD2410::setBTpassword(const char *passwd) {
  byte cmd[10];
  for (unsigned int i = 0; i < 4; i++) cmd[i] = LD2410::BTpasswd[i];
